...
CalculateEnrichmentPValue <- function(predictions, results) {
    ...
    contingencyTable <- matrix(c(numSignificantPredictionsThatAreResponsive, numSignificantPredictionsThatAreUnresponsive, numZeroPredictionsThatAreResponsive, 
        numZeroPredictionsThatAreUnreponsive), nrow = 2)
    enrichmentPValue <- stats::fisher.test(contingencyTable, alternative = "greater")[[1]]
