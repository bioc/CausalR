...
MakePredictionsFromCCG <- function(hypothesisnode, signOfHypothesis, network, delta, nodesInExperimentalData = NULL){
  ...
  # Check that the input hypothesis is a possible id. Ids are numbered from one to the number of vertices, so it is 
  # enough to check the hypothesis is not less than one or greater than the number of vertices.
  if (length(hypothesis)!=1 || hypothesis < 1 || hypothesis > vcount(network)){
  ...
  if (delta > 0){
    for (currentStep in 1:delta){
      nn <- neighborhood(network, currentStep, hypothesis, "out")[[1]] # All nodes out to currentStep
  ...
  # interactions[distance[1:(nNodes/2)] > delta & distance[(nNodes/2+1):nNodes] > delta] = NA # Neither NodeX+ and NodeX- are reached within delta (don't actually need to do this as NA is the default value)
