...
GetApproximateMaximumDValueFromTwoByTwoContingencyTable <- function(n_pp, n_pm, n_mp, n_mm, predictionListStats, experimentalDataStats,
                                                                    logOfFactorialOfPredictionListStats, returnlog = FALSE){
  # A function that will compute an approximate maximum D value (or weight).
  # The reason it is described as approiximate is that this function returns the first valid D value that is found. This has been
  # done to speed up the overall algorithm - to get the exact answer use GetMaximumDValueFromTwoByTwoContingencyTable.
  ...
  # Some of the combinationations produce an invalid contingency table - we can check this by
  # seeing if any of the values in the table are negative.
