...
ReadExperimentalData <- function(fileName, network, removeDuplicates = TRUE){ 
  ...
  if (!(is.igraph(network) || is.null(network))){
    stop('network input must be an igraph')
  }
  
  table <- read.table(fileName, colClasses = "character")
  ...
  if (is.null(network)){
    # No network info provided - return the raw table
    # (Note that this option is provided purely for testing purposes,
    #  e.g. to make sure that the table is being read in correctly, and
    #  is not intended to be used by an end user.)
    return(table)
  }
  else{
