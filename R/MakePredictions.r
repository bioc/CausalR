...
MakePredictions <- function(hypothesisnode, signOfHypothesis, network, delta, nodesInExperimentalData = NULL) {
    # Create a matrix of predictions for a particular hypothesis.  The output is an array containing the relationship between each node and the
    # hypothesis.  The hypothesis provided will be the vertex id of one of the nodes in the network (as an integer node ID or name, including + or - for
    # up/down regulation in the case of a CCG).  The signOfHypothesis variable should be a 1 or -1, indicating up/down regulation.
