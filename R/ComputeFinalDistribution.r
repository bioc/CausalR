...
# *** Summary ***
# Description: Takes the matrix which contains the scores and weights for the different n++, n+- etc.
# values and combines those with the same score to get a matrix which represents the distribution of 
# what scores the hypothesis can have under certain restrictions

# resultsTable contains the scores and weights. We are no longer interest in the individual values of 
# n++, n+- etc., we just want to find the distribution of scores.

# When resultsMatrix is only one row (two values), the max and min functions below don't work because the
# array has been changed to a vector. To put it back into the form of a matrix the following if command is
# used
