...
GetMatrixOfCausalRelationships <- function(hypothesis, network, idsOfConnectedNodesFromSubgraph) {
    ...
    for (iNode in 1:nNodes) {
        if (hypothesis == idsOfConnectedNodesFromSubgraph[iNode]) {
            ...
            shortestPaths <- igraph::all_shortest_paths(network, hypothesis, idsOfConnectedNodesFromSubgraph[iNode], "out")
            ...
            if (numberOfShortestPaths > 1) {
                # if there is more than one shortest path, two things can happen: 1. they could all agree, in which case we can take any of the values in the vector
                # to populate the data frame 2. they could disagree, in this case the interaction given the value 0 (as opposed to 1/-1 for activation/inhibition.
