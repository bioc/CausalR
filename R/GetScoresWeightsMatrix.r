...
GetScoresWeightsMatrix <- function(matrixOfPossibleValues, predictionDataStats, experimentalDataStats, logOfFactorialOfPredictionListStats){
  ...
  for(counter in 1:numRows){
    ...
    logOfWeight = GetWeightForNumbersOfCorrectandIncorrectPredictions(matrixOfPossibleValues[counter,1], matrixOfPossibleValues[counter,2], matrixOfPossibleValues[counter,3], matrixOfPossibleValues[counter,4], 
                    predictionDataStats, experimentalDataStats, logOfFactorialOfPredictionListStats, returnlog =TRUE)
    scoresandLogOfWeightsMatrix[counter,2] <- logOfWeight
  }
  
  # Check total weight
  # expectedWeight = factorial(sum(predictionDataStats))/factorial(numUpregulatedGenes)*factorial(numDownregulatedGenes)*factorial(numNonResponsiveGenes)
