...
DetermineInteractionTypeOfPath <- function(network, nodesInPath) {
    # network must be an igraph nodesInPath is an ordered list of the nodes visited on the path - note that these contain numbers which use R's internal
    ...
    for (i in 1:numEdges) {
        ...
        nodeEdges[i] <- igraph::E(network)[igraph::`%->%`(nodesInPath[[1]][i], nodesInPath[[1]][i + 1])][[1]]
    }
    # Take the product of the weights for each edge in the path to get the overall interaction
    netPathInteraction = prod(igraph::edge_attr(network, "Weight", nodeEdges))
