...
test_GetApproximateMaximumDValueFromThreeByTwoContingencyTable <- function(){
  ...
  checkEquals(CausalR:::GetApproximateMaximumDValueFromThreeByTwoContingencyTable(threeByTwoContingencyTable,                                                                     predictionListStats, logOfFactorialOfPredictionListStats), 108000)
}
...
test_GetMaxDValueForAThreeByTwoFamily <- function(){
  ...
  returnlog <- FALSE
  checkEquals(CausalR:::GetMaxDValueForAThreeByTwoFamily(1,3,1,3,2, 
                                               predictionListStats, logOfFactorialOfPredictionListStats, 
                                               returnlog = FALSE), 24)
}
...
test_CorrectWeightsAboveHypothesisScoreAreReturnedWithDifferentScores <- function(){
  ...
  logDMax <- log(24)
  weightStore <- c(0,0) 
  checkEquals(CausalR:::GetWeightsAboveHypothesisScoreForAThreeByTwoTable(weightStore,1,3,1,3,2, 
                                                                predictionListStats, experimentalDataStats, logOfFactorialOfPredictionListStats, 
                                                                hypothesisScore, logepsDMax, logDMax), c(3, 10/3))
  hypothesisScore2 <- 0
  checkEquals(CausalR:::GetWeightsAboveHypothesisScoreForAThreeByTwoTable(weightStore,1,3,1,3,2, 
                                                                predictionListStats, experimentalDataStats, logOfFactorialOfPredictionListStats, 
                                                                hypothesisScore2, logepsDMax, logDMax), c(2,10/3))
  hypothesisScore3 <- 2
  checkEquals(CausalR:::GetWeightsAboveHypothesisScoreForAThreeByTwoTable(weightStore,1,3,1,3,2, 
                                                                predictionListStats, experimentalDataStats, logOfFactorialOfPredictionListStats, 
                                                                hypothesisScore3, logepsDMax, logDMax), c(1,10/3))
...
test_CubicAlgorithm1bProducesApproximatelyCorrectAnswerWithLargeInputs  <- function(){
  # Check that the answer from the faster cubic algorithm is within a certain tolerance of the answer obtained
